...
async def get_catalog_items(
    ...
    base_query = db.query(Catalog)

    if query_params.search_code:
        base_query = base_query.filter(func.lower(Catalog.code).like(f"%{query_params.search_code.lower()}%"))

    if query_params.search_shape:
        base_query = base_query.join(Catalog.shape).filter(func.lower(Shape.shape).like(f"%{query_params.search_shape.lower()}%"))
        
    if query_params.search_dimensions:
        base_query = base_query.filter(func.lower(Catalog.dimensions).like(f"%{query_params.search_dimensions.lower()}%"))

    if query_params.search_machine:
        base_query = base_query.join(Catalog.equipment_codes) \
    ...
    if query_params.bond_ids:
        base_query = base_query.join(Catalog.bond_to_codes).filter(BondToCode.bond_id.in_(query_params.bond_ids))

    if query_params.grid_size_ids:
        base_query = base_query.filter(Catalog.grid_size_id.in_(query_params.grid_size_ids))

    if query_params.mounting_ids:
        base_query = base_query.filter(Catalog.mounting_id.in_(query_params.mounting_ids))

    if query_params.category_id:
        base_query = base_query.filter(Catalog.category_id == query_params.category_id)
        logger.info(f"Filtering by category_id: {query_params.category_id}")
